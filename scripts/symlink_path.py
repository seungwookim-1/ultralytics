...
VIRTUAL_ROOT = Path("/ultralytics/run/sd_chimera")
...
def _prepare_virtual_dirs():
    for split_dir in (VIRTUAL_IMAGES_TRAIN, VIRTUAL_IMAGES_VAL, VIRTUAL_LABELS_TRAIN, VIRTUAL_LABELS_VAL):
        if split_dir.exists():
            print(f"[ChimeraYolo] 기존 split 삭제: {split_dir}")
...
def _scan_files(root: Path, exts=(".jpg", ".jpeg", ".png", ".txt"), max_files: int | None = None) -> dict[str, Path]:
    ...
    count = 0
    print(f"[ChimeraYolo] 스캔 시작: {root}")
    for p in root.rglob("*"):
        if not p.is_file():
            continue
        ...
        count += 1
        if count % 10000 == 0:
            print(f"[ChimeraYolo] {root}: {count}개 파일 검사 중...")

    print(f"[ChimeraYolo] 스캔 완료: {root}, 매핑 {len(mapping)}개")
    return mapping
...
def _sample_paired_from_labels(
    ...
    if not label_paths:
        print(f"[ChimeraYolo] 라벨 디렉토리 비어있음: {label_root}")
        return {}, {}
    ...
    print(
        f"[ChimeraYolo] {image_root.name}: 라벨 기반 페어 {len(image_map)}개 확보 "
...
def create_dataset_config(val_ratio: float = 0.1,
    ...
    print(
        f"[ChimeraYolo] split: train {len(train_stems)}개, val {len(val_stems)}개 "
    ...
    print(f"[ChimeraYolo] 링크 생성 결과: train {train_count}개, val {val_count}개")
...
"""
    ...
    print(f"[ChimeraYolo] data.yaml 생성 완료: {DATASET_YAML}")
