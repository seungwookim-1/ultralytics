...
results = model.train(
    data='coco.yaml',
    aux_loss_weight=0.05,  # Higher = more balanced experts
)

# Create custom variants by editing YAML:
# - num_experts: 8      # Use more experts
# - aux_loss_weight: 0.02  # Adjust load balancing
```
...
**Solution:** Increase `aux_loss_weight` to 0.05 or 0.1

### One Expert Dominates

**Symptom:** One expert >60% usage
**Solution:** Increase `aux_loss_weight` and train longer
...
3. **YAML Configuration:**
    ...
    - Add `num_experts` and `aux_loss_weight` parameters
